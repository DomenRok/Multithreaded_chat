...
def client_thread(client_sock, client_addr):
        ...
        while True:  # neskoncna zanka
            while not username:
                try:
                    ...
                    clients_usernames[client_sock] = username
                except:
                    print("try again")
                    continue
            ...
            print(f'[{clients_usernames[client_sock]}] [{client_addr[0]}:{str(client_addr[1])}]: {msg_received}')

            for client in clients:
                send_message(client, f'({clients_usernames[client_sock]}) {msg_received}')
    except:
        pass
    ...
    with clients_lock:
        ...
        clients_usernames.pop(client_sock, None)
    print("[system] we now have " + str(len(clients)) + " clients")
    client_sock.close()
